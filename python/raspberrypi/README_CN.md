...
要使用这个库，首先将库下载到Raspberry Pi，然后打开例程文件夹。要执行一个例程demo_x.py，请在命令行中输入python demo_x.py。例如，要执行demo_scan_sensor.py例程，你需要输入:

```python
python demo_scan_sensor.py 
或 
python2 demo_scan_sensor.py 
或 
python3 demo_scan_sensor.py
```
